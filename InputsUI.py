...
def UserInput():
    ...
    while True:
        ...
        if event == "Submit":
            ...
            try:
                datetime.datetime.strptime(values["date_input"], "%m/%d")
            except:
                errorWindow("Please enter a valid date format \"MM/DD\" ")
                continue
            if datetime.datetime.strptime(values["date_input"], "%m/%d") < datetime.datetime.now():
                values["date_input"] = datetime.datetime.now().strftime("%m/%d")
                print(values["date_input"])
                break
